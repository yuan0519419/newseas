  ...
  <div v-if="dataLoaded">
    <div class="button-container">
      <el-date-picker
          ...
          >查询</el-button
          >
      <el-button type="success" size="large" @click="DataRefresh">
        刷新数据
      </el-button>
    </div>
...
const initChart = (domId, title, dataKey) => {
  if (sea.value && sea.value.length > 0) {
    dataLoaded.value = true; // 先更新数据加载状态，确保DOM已渲染
    nextTick(() => {
      initData(sea.value, domId, title, dataKey);
    });
  } else {
...
const initData = (data, domId, title, dataKey) => {
  ...
  // 排序和截取数据
  const sortedData = data.sort((a, b) => new Date(a.sampleTime) - new Date(b.sampleTime));
  ...
  const option = {
    ...
    xAxis: {
      data: sortedData.map(item => item.sampleTime),
      axisLabel: {
        formatter: (value) => format.formatTime('MM-dd hh:mm:ss', value)
      }
    },
    yAxis: {},
    series: [
...
const refreshCharts = () => {
  initChart('o-chart', '溶解氧(mg/L)', 'o');
  initChart('nh-chart', '氨氮(mg/L)', 'nh');
  initChart('no-chart', '亚硝酸氮(mg/L)', 'no');
  initChart('temp-chart', '水温(℃)', 'temp');
  initChart('salinity-chart', '盐度(PSU)', 'salinity');
  initChart('turbidity-chart', '浊度(NTU)', 'turbidity');
  initChart('ph-chart', 'pH值', 'ph');
  initChart('chl-chart', '叶绿素a(μg/L)', 'chl');
  initChart('currVel-chart', '洋流流速(cm/s)', 'currVel');
  initChart('orp-chart', '氧化还原电位（mv）', 'orp');
};
